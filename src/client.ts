...
export class InworldTTS {
    ...
    async *speakStream(
        ...
        // (1) Build the same payload as `speak()`
        const payload: Record<string, any> = { text, ...options };
        if (!payload.modelId) payload.modelId = this.defaultModel;
        if (!payload.format) payload.format = 'mp3';
        ...
        // (3) Pull from the ReadableStream
        ...
        while (true) {
            ...
            // (4) Decode Uint8Array to string, split on newline
            ...
            while ((idx = buf.indexOf('\n')) !== -1) {
                ...
                try {
                    ...
                    console.warn('[speakStream] Failed to parse line:', line);
                    continue;
                }
                const { result, error } = parsed;
                if (error && error.message && (!result || !result.audioContent)) {
